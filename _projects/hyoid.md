---
layout: page
title: Tuamotu Sandpiper and Hyoid Morphology
description: Exploring a curious case of nectarivory
img: assets/img/project_photos/tuamotu_cover.jpg
importance: 3
category: past
---

The Tuamotu Sandpiper (<i>Prosobonia parvirostris</i>) is a unique species of shorebird native to the Tuamotu Archipelago of French Polynesia. It has been observed to feed on nectar, a behavior that is exceedingly rare for shorebirds. To investigate the morphological precursors that enable this adaptation (and similar dietary habits like biofilm feeding), we took morphological measurements of the hyoid, bill and skull from species across the Scolopacidae family, and compared these with dietary categorizations.


<div class="row">
    <div class="col-sm mt-3 mt-md-0">
        {% include figure.liquid loading="eager" path="assets/img/project_photos/hyoid.png" title="example image" class="img-fluid rounded" %}
    </div>
</div>
<div class="caption">
    The tiny hyoid bone of a Semipalmated Sandpiper (<i>Calidris pusilla </i>)!
</div>


<div class="caption">
    Cover photo: A Tuamotu sandpiper (<i>Prosobonia parvirostris</i>). Credit: James Eaton, accessed from Avicommons.
</div>